...
function TroubleSignalPanel({ trouble }: { trouble: HangaraeTrouble }) {
  const stages = trouble.metricBody
    .split("->")
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_2fr_1fr]">
      ...
      <div className="rounded-[28px] bg-[#F8FAFC] p-6">
        ...
        <p className="mt-4 text-[20px] font-semibold leading-[1.2] tracking-[-0.03em] text-[#111827] md:text-[24px]">
          {trouble.metricTitle}
        </p>
      </div>
      ...
      <div className="rounded-[28px] border border-[#E2E8F0] bg-white p-6">
        <p className="small-label mb-4 text-[#22C55E]">Flow</p>
        <div className="flex flex-wrap items-center gap-2">
          {stages.map((stage, index) => (
            <div key={`${trouble.id}-flow-${stage}`} className="flex items-center gap-2">
              <span className="rounded-full border border-[#DCE5EF] bg-[#F8FAFC] px-3.5 py-2 text-sm font-semibold text-[#334155]">
                {stage}
              </span>
      ...
      <div className="flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
          <div className="rounded-[24px] border border-[#E2E8F0] bg-white p-5">
            <p className="small-label text-slate-500">Debug Units</p>
            <p className="mt-3 text-[28px] font-semibold leading-none tracking-[-0.05em] text-[#111827]">
              {stages.length}
            </p>
            <p className="project-caption mt-1.5">stages in this flow</p>
          </div>
          <div className="rounded-[24px] border border-[#E2E8F0] bg-white p-5">
            <p className="small-label text-slate-500">Tech Touchpoints</p>
            <p className="mt-3 text-[28px] font-semibold leading-none tracking-[-0.05em] text-[#111827]">
              {trouble.tech.length}
            </p>
            <p className="project-caption mt-1.5">implementation surfaces</p>
          </div>
        </div>
        <div className="rounded-[24px] bg-[#F8FAFC] p-5">
          <p className="small-label text-[#22C55E]">Tech Focus</p>
          <div className="mt-3 flex flex-wrap gap-2">
            {trouble.tech.map((item) => (
              <span
                key={item}
                className="rounded-full border border-[#DCE5EF] bg-white px-3 py-2 text-sm font-semibold text-slate-700"
              >
                {item}
              </span>
            ))}
